...
func TestAddWithExtremelyLongParentChain(t *testing.T) {
	...
	for i := 0; i < chainLength; i++ {
		...
		id, err := store.Add("Deep", dimensions)
		if err != nil {
...
func TestUpdateWithNonExistentParent(t *testing.T) {
	...
	err = store.Update(docID, nanostore.UpdateRequest{
		Dimensions: map[string]interface{}{"parent_uuid": nonExistentParent},
	})
	if err == nil {
...
func TestConcurrentCircularReferenceCheck(t *testing.T) {
	...
	bID, _ := store.Add("B", map[string]interface{}{"parent_uuid": aID})
	cID, _ := store.Add("C", map[string]interface{}{"parent_uuid": bID})
