...
func (tq *TypedQuery[T]) OrderBy(column string) *TypedQuery[T] {
	tq.options.OrderBy = append(tq.options.OrderBy, OrderClause{
		Column:     column,
		Descending: false,
	})
	return tq
...
func generateConfigFromType(typ reflect.Type) (Config, error) {
	var config Config
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if tagValue := field.Tag.Get("values"); tagValue != "" {
			...
			if isRef {
				...
				config.Dimensions = append(config.Dimensions, DimensionConfig{
